  ...
  def forward(self, input, hx=(None, None, None), reset_experience=False, pass_through_memory=True, flag_list=None, additiona_vec=T.from_numpy(np.array([0,1]).astype("float32")), ctr_penalty=0.2):
    ...
    for time in range(max_length):
      ...
      if flag_list is not None:
        if flag_list[time] == 2:
          # print(time, inputs[time][0][:self.input_size])
          _t, a_inputs = T.split(inputs[time], [self.whole_output_size, inputs[time].size()[1]-self.whole_output_size], dim=1)
          # inputs[time] = T.cat([F.softmax(self.output(outs[time-1])), a_inputs], 1)
          inputs[time] = T.cat([self.output(outs[time-1]).tanh(), a_inputs], 1)
          # print(time, inputs[time][0][:self.input_size])
          # print("\n")
